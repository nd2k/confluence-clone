<script lang="ts">
    ...
    $: style = outline ? 
	`--rounded: ${rounded}%; --width: ${width}rem; --height: ${height}rem; --background: ${color}; 
	...
	: `--rounded: ${rounded}%; --width: ${width}rem; --height: ${height}rem; --background: ${backgroundColor}; 
...
<button on:click={emitEvent} {style} {disabled} class={active ? 'active' : ''}>
    <slot/>
</button>
